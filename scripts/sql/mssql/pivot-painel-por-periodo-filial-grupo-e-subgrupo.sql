        ...
        SET @COMANDO_SQL = '
             SELECT t.RD_DATARQ
			  ...
			  (			        
                ...
                GROUP BY t.RD_DATARQ,t.RD_FILIAL,t.'+@COLUNAS_PIVOT+'
			 ) t
             GROUP BY t.RD_DATARQ,t.RD_FILIAL'
            BEGIN TRANSACTION @TTSNAME
                EXECUTE('SELECT * INTO '+@TABLENAME+' FROM ('+@COMANDO_SQL+') t') 
            COMMIT TRANSACTION @TTSNAME
	END TRY
	BEGIN CATCH
			EXECUTE('SELECT * INTO '+@TABLENAME+' FROM (SELECT '''' RD_DATARQ, '''' RD_FILIAL,0 TOTAL ) t')	
	END CATCH;
END