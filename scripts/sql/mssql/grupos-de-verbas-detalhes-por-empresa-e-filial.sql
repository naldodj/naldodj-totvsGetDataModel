BEGIN
    ...
    BEGIN TRY
        ...
        SET @COMANDO_SQL = '
              ...
              FROM (
                    ...
                    (SELECT * FROM
                        (
                         ...
                         GROUP BY SRD.RD_FILIAL,SRD.RD_DATARQ,ZY_.ZY__MASTER,ZY_.ZY__SQLFLD,ZY_.ZY__SQLFLD
                        ) ROW
                    PIVOT (SUM(ROW.RD_VALOR) FOR ZY__SQLFLD IN (' + @COLUNAS_PIVOT + ')) COL ) t
                    GROUP BY t.RD_FILIAL,t.RD_DATARQ,t.ZY__MASTER,t.'+@COLUNAS_PIVOT+') t
             ...
             GROUP BY t.RD_FILIAL,t.RD_DATARQ,t.ZY__MASTER,ZY_.ZY__DESC'
            BEGIN TRANSACTION @TTSNAME
                EXECUTE('SELECT * INTO '+@TABLENAME+' FROM ('+@COMANDO_SQL+') t')
            COMMIT TRANSACTION @TTSNAME
	END TRY
	BEGIN CATCH
			EXECUTE('SELECT * INTO '+@TABLENAME+' FROM (SELECT '''' RD_FILIAL,'''' RD_DATARQ,'''' ZY__MASTER,'''' ZY__DESC,0 TOTAL ) t')
	END CATCH;
END