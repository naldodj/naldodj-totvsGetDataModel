...
BEGIN TRY
        ...
        SET @COMANDO_SQL = '
              ...
              FROM (			        
                    ...
                    (SELECT * FROM 
                        (
                         ...
                         GROUP BY SRD.RD_FILIAL,SRD.RD_DATARQ,SRA.RA_MAT,SRA.RA_NOME,RA_ADMISSA,RD_CC,CTT.CTT_DESC01,ZY_.ZY__MASTER
                        ) ROW
                    PIVOT (SUM(ROW.RD_VALOR) FOR ROW.RD_DATARQ IN (' + @COLUNAS_PIVOT + ')) COL ) t
                    GROUP BY t.RD_FILIAL,t.RD_CC,t.CTT_DESC01,t.RA_MAT,t.RA_NOME,t.RA_ADMISSA,t.ZY__MASTER,t.'+@COLUNAS_PIVOT+') t
             ...
             GROUP BY t.RD_FILIAL,t.RD_CC,t.CTT_DESC01,t.RA_MAT,t.RA_NOME,t.RA_ADMISSA,t.ZY__MASTER,ZY_.ZY__DESC'
            BEGIN TRANSACTION @TTSNAME
                EXECUTE('SELECT * INTO '+@TABLENAME+' FROM ('+@COMANDO_SQL+') t') 
            COMMIT TRANSACTION @TTSNAME
	END TRY
BEGIN CATCH
			EXECUTE('SELECT * INTO '+@TABLENAME+' FROM (SELECT '''' RD_FILIAL,'''' RD_CC,'''' CTT_DESC01,'''' RA_MAT,'''' RA_NOME,'''' RA_ADMISSA,'''' ZY__MASTER,'''' ZY__DESC,0 TOTAL ) t')	
	END CATCH;
END