              ...
              FROM (			        
                    ...
                    (SELECT * FROM 
                        (
                         ...
                         GROUP BY SRD.RD_FILIAL,SRD.RD_DATARQ,ZY_.ZY__MASTER,ZY_M.ZY__SQLFLD,ZY_.ZY__SQLFLD
                        ) ROW
                    PIVOT (SUM(ROW.RD_VALOR) FOR ZY__SQLFLD IN (' + @COLUNAS_PIVOT + ')) COL ) t
                    GROUP BY t.RD_FILIAL,t.RD_DATARQ,t.'+@COLUNAS_PIVOT+') t
             GROUP BY t.RD_FILIAL,t.RD_DATARQ'
            BEGIN TRANSACTION @TTSNAME
                EXECUTE('SELECT * INTO '+@TABLENAME+' FROM ('+@COMANDO_SQL+') t') 
            COMMIT TRANSACTION @TTSNAME
	END TRY
	BEGIN CATCH
			EXECUTE('SELECT * INTO '+@TABLENAME+' FROM (SELECT '''' RD_FILIAL,'''' RD_DATARQ,SUBTOTAL=0 ,PROV13S=0,PROVFER=0,PROVRES=0,PROVTOT=0,TOTAL=0 ) t')	
	END CATCH;
END