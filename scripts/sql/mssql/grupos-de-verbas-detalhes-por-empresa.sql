                    ...
                    (SELECT * FROM 
                        (
                            ...
                            WHERE SRD.D_E_L_E_T_='''' 
                              ...
                              AND ZY_.ZY__MASTER BETWEEN '''+@GRUPODE+''' AND '''+@GRUPOATE+'''
                              AND ZY_.ZY__SQLFLD<>''''
                         GROUP BY SRD.RD_DATARQ,ZY_.ZY__MASTER,ZY_.ZY__SQLFLD,ZY_.ZY__SQLFLD
                        ) ROW
                    PIVOT (SUM(ROW.RD_VALOR) FOR ZY__SQLFLD IN (' + @COLUNAS_PIVOT + ')) COL ) t
                    GROUP BY t.RD_DATARQ,t.ZY__MASTER,t.'+@COLUNAS_PIVOT+') t
             JOIN ZY_990 ZY_ ON (ZY_.ZY__CODIGO=t.ZY__MASTER AND ((CASE ZY_.ZY__FILIAL WHEN '''' THEN 1 WHEN '''+@FILIALDE+''' THEN 1 ELSE 0 END)=1) )
             GROUP BY t.RD_DATARQ,t.ZY__MASTER,ZY_.ZY__DESC'
            BEGIN TRANSACTION @TTSNAME
                EXECUTE('SELECT * INTO '+@TABLENAME+' FROM ('+@COMANDO_SQL+') t') 
            COMMIT TRANSACTION @TTSNAME
	END TRY
	BEGIN CATCH
			EXECUTE('SELECT * INTO '+@TABLENAME+' FROM (SELECT '''' RD_DATARQ,'''' ZY__MASTER,'''' ZY__DESC,0 TOTAL ) t')	
	END CATCH;
END